    ...
    void Update()
    {
        if (owner != null)
        {
            ...
            else
            {
                ...
                xPos = Mathf.Clamp(Camera.main.WorldToScreenPoint(owner.transform.position + Vector3.up * 5).x, 30 * mainCanvas.localScale.x, (mainCanvas.rect.width - 30) * mainCanvas.localScale.x);
                yPos = Mathf.Clamp(Camera.main.WorldToScreenPoint(owner.transform.position + Vector3.up * 5).y, 30 * mainCanvas.localScale.y, (mainCanvas.rect.height - 30) * mainCanvas.localScale.y);
